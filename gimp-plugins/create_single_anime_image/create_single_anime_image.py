...
"""
AnimeListCommander - Single Anime Image Generator Plugin
...
def get_flattened_drawable(file_path):
    ...
    try:
        ...
        # 【プロンプト内のコードは適宜最適化してください！】
        # GIMP 3.2.2ではflatten()の戻り値が異なる場合があるため、レイヤー取得を確実に行う
        flattened_layer = img.flatten()
        return img, flattened_layer
    except: return None
...
class CreateSingleAnimeImage(Gimp.PlugIn):
    ...
    def run(self, procedure, run_mode, kv_image, drawables, config, run_data):
        ...
        if not os.path.exists(MACRO_SETTINGS_PATH):
            show_message_dialog(f"共通マクロ設定が見つかりません:\n{MACRO_SETTINGS_PATH}\n環境変数 GIMP_ANIME_MACRO_SETTINGS または DEFAULT_MACRO_PATH を確認してください。", "エラー", True)
            return procedure.new_return_values(Gimp.PDBStatusType.CANCEL, GLib.Error())
        ...
        sync_targets = [
            ("KEY_VISUAL", kv_path), 
            ("PRODUCTION_LOGO", "COMPANY_LOGO_PATH", "UNRELEASED_COMPANY_LOGO"), 
            ("BROADCAST_LOGO", "BROADCAST_LOGO_PATH", "UNRELEASED_BROADCAST_LOGO")
        ]
        ...
        for key, base_size in text_map.items():
            ...
            if lines:
                ...
                if key == "STAFF":
                    markup = "\n".join([f'<span size="{int((13 if i%2!=0 else 10)*1024)}">{l.replace("&","&amp;").replace("<","&lt;")}</span>' for i,l in enumerate(lines)])
                    ...
                    fs = base_size
                    if key=="TITLE":
                        if has_ruby: fs=15; offset_layer_y(t_layer,5)
                        else:
                    ...
                    elif key=="CAST": fs=base_size if (len(lines)==1 and lines[0]=="キャスト未発表") else (12 if len(lines)==14 else (10 if len(lines)>=15 else 13))
            ...
            else: t_layer.set_visible(False)
